...
exports.command = {
    ...
    init: {
        ...
        run: function () {
            ...
            fs.readFile(
                pathUtil.resolve(__dirname, watchreload.cusotmConfigTplFile),
                function (err, data) {
                    if (err) {
                        console.err(err);
                        return;
    ...
    start: {
        usage: 'watchreload start [--port <port>] [--configFile <filePath>]',
        description: '启动监控服务器',
