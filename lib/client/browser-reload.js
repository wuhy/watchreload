...
;(function (window, document, io, reloader) {
    ...
    reloader.util = {
         ...
         * @return {string}
         */
        addQueryTimestamp: function (url) {
            var urlInfo = reloader.util.parseURL(url);
            var param = urlInfo.param;
            var versionParam = 'browserreload=' + (new Date()).getTime();
        ...
        /**
         * 从给定的文件列表里，查找文件路径跟要查找的目标文件路径最大匹配的文件
         *
        ...
        findMaxMatchFile: function (path, fileInfoList) {
            ...
            for (var i = 0, len = fileInfoList.length; i < len; i++) {
                ...
                if (!maxMatch || (maxMatch.ratio < ratio)) {
    ...
    reloader.dom = {
        ...
        extractImportStyleHref: function (href) {
            ...
            var extractURLResult = /^url\s*\(\s*['"]?\s*([^\s]*)\s*['"]?\s*\)/.exec(href);
    ...
    reloader.command = {
        ...
        reloadCSS: function (data) {
            ...
            var maxMatchLink = findMaxMatchFile(changeFilePath, linkStyles);
            var maxMatchImport = findMaxMatchFile(changeFilePath, importStyles);

            if (maxMatchLink.isFullMatch) {
                reloader.dom.reloadLinkStyle(maxMatchLink.file);
                hasMatch = true;
            }
            if (maxMatchImport.isFullMatch) {
    ...
    function log(type, msg) {
        ...
        var options = reloader.options;
        if ((options.debug || isError)
            && console
            ) {
            var log = console[typeMap[type]] || console.log;
            if (typeof log === 'function') {
                log.call(console, '[browser-reload-client-' + type + ']: ' + msg);
            }
        }
    }
...
})(window, document, io, window._browserReloader || (window._browserReloader = {}));
