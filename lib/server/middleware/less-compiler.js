...
exports.compile = function (callback, content, options, req, res) { // jshint ignore:line
    ...
    // 初始化 less 查询路径
    var confPaths = compileOption.paths || [];
    [rootPath, pathUtil.join(rootPath, pathUtil.dirname(reqPath))].forEach(function (item) {
        if (confPaths.indexOf(confPaths, item) === -1) {
            confPaths.push(item);
        }
    });
    compileOption.paths = confPaths;

    // 重新设置响应的内容类型为css
    res.setHeader('content-type', 'text/css; charset=UTF-8');

    less.render(content.toString(), compileOption, function (err, result) {
        callback(err, err ? null : result.css);
    });
};
