...
function WatchServer(options) {
    ...
    // 初始化配置文件
    if (configFile) {
        configFile = helper.resolvePath(options.basePath, configFile);
    }
    else {
        configFile = helper.resolvePath(options.basePath, WATCHER_CONFIG_FILE);
    }
 ...
 * @return {http.Server}
 */
WatchServer.prototype.createWebServer = function () {
    var app = connect();
    ...
    var url = 'http://' + this.ip + ':' + this.port + '/' + clientConf.name;
    app.use(watchreloadInjector({
        snippet: '<script src="' + url + '"></script>'
    }));

    // 初始化代理中间件
    var proxyInfo = this.options.proxy;
    var proxyMiddleware;
    ...
    if (!proxyMiddleware) {
        app.use(staticServer({
            path: helper.resolvePath(basePath, '.')
        }));
    }
...
WatchServer.prototype.start = function (callback) {
    ...
    var httpServer = this.createWebServer();
