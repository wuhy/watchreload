...
WatchServer.prototype.readWatchConf = function (configFile) {
    ...
    var watcherOptions = {};
    var optionConfigPath = pathUtil.resolve(this.configFile);
    var customOption;

    try {
        // 删除缓存
        delete require.cache[require.resolve(optionConfigPath)];
        customOption = require(optionConfigPath);
    }
    catch (ex) {
        customOption = {};
    }
 ...
 *                 要监控的文件
 */
WatchServer.prototype.initWatchFiles = function (files) {
    files || (files = {});
    var watchFiles = files.include || [];
    watchFiles.push(this.configFile);
    files.include = watchFiles;
